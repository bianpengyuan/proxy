...
ServiceControlHandlerImpl::ServiceControlHandlerImpl(
  ...
  if (!operation.empty()) {
    ...
    if (!require_ctx_) {
      ENVOY_LOG(debug, "bianpengyuan No requirement matched!");
    }
  } else {
    ENVOY_LOG(debug, "bianpengyuan No operation found");
  }
  if (require_ctx_ == nullptr) {
    ENVOY_LOG(debug, "bianpengyuan Use non matched requirement.");
...
absl::string_view ServiceControlHandlerImpl::getOperationFromPerRoute(
  ...
  if (stream_info_.routeEntry() == nullptr) {
    ENVOY_LOG(debug, "bianpengyuan No route entry");
  ...
  if (per_route == nullptr) {
    ENVOY_LOG(debug, "bianpengyuan no per-route config");
    return Envoy::EMPTY_STRING;
  }
  ENVOY_LOG(debug, "bianpengyuan get operation_name: {}", per_route->operation_name());
...
void ServiceControlHandlerImpl::callCheck(
  ...
  ENVOY_LOG(debug, "bianpengyuan callcheck 1");
  ...
  ENVOY_LOG(debug, "bianpengyuan callcheck 2");
  ...
  ENVOY_LOG(debug, "bianpengyuan callcheck 3");
  ...
  ENVOY_LOG(debug, "bianpengyuan callcheck 4");
...
void ServiceControlHandlerImpl::callQuota() {
  ENVOY_LOG(debug, "bianpengyuan callQuota 1");
  if (!isQuotaRequired()) {
    check_callback_->onCheckDone(check_status_, rc_detail_);
    return;
  }
  ENVOY_LOG(debug, "bianpengyuan callQuota 2");
  ...
  ENVOY_LOG(debug, "bianpengyuan callQuota 3");
