...
func (c *ClientEnvoy) Run(p *Params) error {
	u := Update{
		Node:      "client",
		Version:   "0",
		Listeners: []string{p.LoadTestData("testdata/listener/client.yaml.tmpl")},
	}
...
func (s *ServerEnvoy) Run(p *Params) error {
	u := Update{
		Node:      "server",
		Version:   "0",
		Listeners: []string{p.LoadTestData("testdata/listener/server.yaml.tmpl")},
	}
	if err := u.Run(p); err != nil {
		return err
	}
	...
	s.httpBackend, err = env.NewHTTPServer(p.Ports.BackendPort, false, "")
	if err != nil {
